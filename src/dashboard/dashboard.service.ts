...
export class DashboardService {
  ...
  async getDashboardStats() {
    ...
    const projectRepository =
      this.clientProfileRepository.manager.getRepository(ProjectEntity);
    const projects = await projectRepository.find({
      relations: { client: true, tasks: true },
    });

    const tasksRepository =
      this.clientProfileRepository.manager.getRepository(TaskEntity);
    const totalTasksCount = await tasksRepository.count();
    const completedTasksCount = await tasksRepository.count({
    ...
    const topProjects = projects.map((p) => ({
      ...
      completedTasks: p.tasks
        ? p.tasks.filter((t) => t.status === TaskStatus.DONE).length
        : 0,
